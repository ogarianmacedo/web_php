...
if($arquivo == ""){
	...
	$query = mysqli_query($conectar, "UPDATE tb_produto SET nome = '$nome',	resumo = '$resumo',	descricao = '$descricao', valor = '$valor',	tag = '$tag', description = '$description', situacao_id = '$situacao_id',	categoria_id = '$categoria_id', modificado = NOW() WHERE id_produto = '$id_produto'");

	if(mysqli_affected_rows($conectar) != 0){
		echo "<script type='text/javascript'>
		swal(\"Muito bem!\", \"Produto alterado com sucesso.\", \"success\");
		</script>";
		echo "<META HTTP-EQUIV=REFRESH CONTENT = '0;URL=http://localhost/web_php/app-adm/views/administrativo.php?link=12'>";
	}else{
		echo "<script type='text/javascript'>
		swal(\"Ops!\", \"Produto não alterado.\", \"danger\");
		</script>";
		echo "<META HTTP-EQUIV=REFRESH CONTENT = '0;URL=http://localhost/web_php/app-adm/views/administrativo.php?link=12'>";
	}	
	...
	if(array_search($extensao, $_UP['extensoes']) === false){
		...
		if(mysqli_affected_rows($conectar) != 0){
			...
			echo "<script type='text/javascript'>
			swal(\"Ops!\", \"A imagem não foi alterada, utilize imagens nos seguinte formatos: png, jpg, jpeg e gif.\", \"warning\");
			</script>";
			echo "<META HTTP-EQUIV=REFRESH CONTENT = '0;URL=http://localhost/web_php/app-adm/views/administrativo.php?link=12'>";
			...
			echo "<script type='text/javascript'>
			swal(\"Ops!\", \"Produto não editado.\", \"danger\");
			</script>";
			echo "<META HTTP-EQUIV=REFRESH CONTENT = '0;URL=http://localhost/web_php/app-adm/views/administrativo.php?link=12'>";
		}
	...
	// Verifica o tamanho do arquivo
		...
		if(move_uploaded_file($_FILES['arquivo']['tmp_name'], $_UP['pasta'].$nome_final)){
			...
			$query = mysqli_query($conectar, "UPDATE tb_produto SET nome = '$nome',	resumo = '$resumo',	descricao = '$descricao', valor = '$valor',	tag = '$tag', description = '$description', arquivo = '$nome_final', situacao_id = '$situacao_id',	categoria_id = '$categoria_id', modificado = NOW() WHERE id_produto = '$id_produto'");

			echo "<script type='text/javascript'>
			swal(\"Muito bem!\", \"Produto editado com sucesso.\", \"success\");
			</script>";
			echo "<META HTTP-EQUIV=REFRESH CONTENT = '0;URL=http://localhost/web_php/app-adm/views/administrativo.php?link=12'>";
